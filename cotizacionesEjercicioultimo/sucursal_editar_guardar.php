...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if($ins->execute()){
        header("Location: aviso.php?tipo=exito&operacion=Sucursal editada&destino=ver_sucursal.php");
    }
    else{
        header("Location: aviso.php?tipo=fracaso&operacion=Sucursal No editado (Sucursal repetida?)&destino=ver_sucursal.php");
    }
    $ins->close();
    $con->close();
